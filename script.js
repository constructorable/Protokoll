...
// Globale Variable für den Kamerastream
let cameraStream = null;

async function setupCameraUpload(uploadButton) {
    uploadButton.addEventListener("click", async function() {
        // Dialog für Kameraoptionen erstellen
        ...
        dialog.innerHTML = `
            <h3 style="margin-top:0">Foto aufnehmen</h3>
            <div style="display:flex; flex-direction:column; gap:10px;">
                <button id="takeSinglePhoto">Einzelnes Foto aufnehmen</button>
                <button id="startMultiPhoto">Mehrere Fotos aufnehmen (Modus)</button>
                <button id="cancelCamera">Abbrechen</button>
            </div>
            <div id="cameraPreview" style="margin-top:20px; display:none;">
                <video id="cameraView" autoplay playsinline style="width:100%; max-height:300px;"></video>
                <div style="display:flex; justify-content:center; gap:10px; margin-top:10px;">
                    <button id="capturePhoto" style="padding:10px 20px;">Foto aufnehmen</button>
                    <button id="stopCamera" style="padding:10px 20px;">Fertig</button>
                </div>
            </div>
        `;
        ...
        // Kamera-Elemente
        const video = dialog.querySelector('#cameraView');
        const cameraPreview = dialog.querySelector('#cameraPreview');
        let isMultiPhotoMode = false;
        
        // Event-Handler für die Buttons
        dialog.querySelector('#takeSinglePhoto').addEventListener('click', async () => {
            await startCamera();
            capturePhoto();
            await stopCamera();
            dialog.remove();
        ...
        dialog.querySelector('#startMultiPhoto').addEventListener('click', async () => {
            isMultiPhotoMode = true;
            await startCamera();
            cameraPreview.style.display = 'block';
        });
        
        dialog.querySelector('#cancelCamera').addEventListener('click', () => {
            stopCamera();
            dialog.remove();
        });
        
        dialog.querySelector('#capturePhoto').addEventListener('click', () => {
            capturePhoto();
        });
        
        dialog.querySelector('#stopCamera').addEventListener('click', async () => {
            await stopCamera();
            dialog.remove();
        });
        ...
        function capturePhoto() {
            const title = uploadButton.getAttribute("data-title");
            const imagePreview = uploadButton.nextElementSibling;
            ...
            scaledCanvas.toBlob(function(blob) {
                ...
                createThumbnail(imagePreview, signContainer, imageUrl, title, storedImages);
...
// Hilfsfunktion zum Erstellen der Thumbnails und großen Bilder
function createThumbnail(imagePreview, signContainer, imageUrl, title, storedImages) {
    // Miniaturansicht mit Löschen-Button
    let imgWrapper = document.createElement("div");
    imgWrapper.style.display = "inline-block";
    imgWrapper.style.position = "relative";
    ...
    deleteButton.addEventListener("click", function() {
        ...
        storedImages = storedImages.filter(img => img.imageUrl !== imageUrl);
        localStorage.setItem('uploadedImages', JSON.stringify(storedImages));
    });
...
// Initialisierung für alle Upload-Buttons
document.querySelectorAll('input[class^="imageUpload"]').forEach(button => {
    // Ersetze den File-Upload-Button durch einen Kamera-Button
    const cameraButton = document.createElement('button');
    cameraButton.className = button.className;
    cameraButton.setAttribute('data-title', button.getAttribute('data-title'));
    cameraButton.textContent = 'Foto aufnehmen';
    cameraButton.style.margin = '5px';
    
    button.parentNode.insertBefore(cameraButton, button);
    button.style.display = 'none';
    
    setupCameraUpload(cameraButton);
});
