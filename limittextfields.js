...
class BemerkungTextLimitMonitor {
    ...
    initializeMonitoring() {
        ...
        bemerkungFields.forEach((field, index) => {
            ...
            console.log(`📝 Feld ${index + 1}:`);
            console.log(`   - ID: "${field.id}"`);
            console.log(`   - Wert: "${fieldValue}"`);
            console.log(`   - Länge: ${fieldLength} Zeichen`);
            ...
            if (fieldLength > this.characterLimit) {
                ...
                console.warn(`   Feld-ID: ${field.id}`);
                console.warn(`   Wert: "${fieldValue}"`);
                console.warn(`   Länge: ${fieldLength} Zeichen (Limit: ${this.characterLimit})`);
                console.warn(`   ❌ ABER MODAL WIRD NICHT ANGEZEIGT (Feld nicht aktiv)`);
            }
    ...
    addFieldMonitoring(inputField) {
        ...
        console.log(`➕ Überwachung hinzugefügt für Feld: ${inputField.id}`);
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        newBemerkungFields.forEach(field => {
                            if (!this.monitoredFields.has(field)) {
                                console.log(`🆕 Neues Bemerkungsfeld gefunden: ${field.id}`);
                                this.addFieldMonitoring(field);
                            }
                        });
                        ...
                        if (node.matches && node.matches('.bemerkung-input')) {
                            if (!this.monitoredFields.has(node)) {
                                console.log(`🆕 Neues direktes Bemerkungsfeld: ${node.id}`);
